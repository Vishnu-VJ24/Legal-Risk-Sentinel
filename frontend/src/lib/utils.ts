...
export const getSeverityBadgeStyles = (severity: string) => {
  switch (normalizeSeverityLabel(severity)) {
    ...
    case 'HIGH': return 'bg-red-500 text-white border-red-600 hover:bg-red-600';
...
export const getSeverityCardStyles = (severity: string) => {
  switch (normalizeSeverityLabel(severity)) {
    case 'CRITICAL': return 'bg-red-600/10 border-red-300 text-red-900';
    case 'HIGH': return 'bg-red-500/10 border-red-200 text-red-900';
