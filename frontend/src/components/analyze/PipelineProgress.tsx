...
export const PipelineProgress = ({ isWarmingUp, progress, stage }: PipelineProgressProps) => {
    ...
    <div className="rounded-[28px] border border-border bg-surface/85 p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {isWarmingUp ? (
            <p className="mt-2 text-sm text-secondary">Warming up AI engine — first load takes ~20s...</p>
          ) : null}
        </div>
      ...
      {isWarmingUp ? (
        <div className="mt-6 rounded-2xl border border-primary/30 bg-primary/10 p-4 text-sm text-text-secondary">
          The Hugging Face Space is waking up. Once the API responds, LexAI will continue automatically.
        </div>
      ) : (
        <motion.div
          initial="hidden"
